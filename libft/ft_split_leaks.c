...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/06 13:55:11 by ctoujana          #+#    #+#             */
/*   Updated: 2025/05/11 10:58:37 by zguellou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_word_len1(char *str, char *c, int *j)
{
	...
	while (str[*j] && (vars.in_s || vars.in_d || !is_sep(str[*j], c)))
	{
		if (str[*j] == '\'' && !vars.in_d)
			...
			vars.in_d = !vars.in_d;
		if ((str[*j] == '\'' && !vars.in_d) || (str[*j] == '"' && !vars.in_s))
			(*j)++;
		else
		{
			vars.len++;
...
static char	*populate_word(int *index, int j, char *str, char *word)
{
	...
	while (*index < j)
	{
		if (str[*index] == '\'' && !vars.in_d)
		{
			vars.in_s = !vars.in_s;
			(*index)++;
		}
		else if (str[*index] == '"' && !vars.in_s)
		{
			vars.in_d = !vars.in_d;
			(*index)++;
		}
...
static char	*get_word(char *str, int *index, char *c)
{
	t_split	vars;
	char	*word;
	int		j;
	...
	start = *index;
	j = start;
	word = malloc((get_word_len1(str, c, &j) + 1));
	if (!word)
		return (NULL);
